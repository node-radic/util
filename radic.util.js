(function (global, factory) {
	...
	(factory((global.radic = global.radic || {}, global.radic.util = {}),global._));
...
var StorageMeta = (function () {
    ...
    StorageMeta.prototype.merge = function (options) {
        lodash.merge(this, options);
        return this;
    };
    StorageMeta.create = function (options) { return new StorageMeta(options); };
    StorageMeta.fromString = function (meta) { return new StorageMeta(JSON.parse(meta)); };
    StorageMeta.prototype.toString = function () { return JSON.stringify({ json: this.json, expires: this.expires }); };
    StorageMeta.prototype.expiresIn = function (minutes) { this.expires = Math.floor((Date.now() / 1000) / 60) + minutes; };
    StorageMeta.prototype.isExpired = function () {
        if (!this.canExpire()) {
            return false;
        }
    ...
    StorageMeta.prototype.canExpire = function () { return this.expires && this.expires !== null && kindOf(this.expires) === "number"; };
    StorageMeta.prototype.isJSON = function () { return this.json === true; };
    StorageMeta.prototype.setJSON = function (val) { this.json = val; };
    return StorageMeta;
}());
var StorageBag = (function () {
    function StorageBag(provider, options) {
        ...
        this.listeners = { set: [], del: [], clear: [] };
        this.provider = provider;
        lodash.merge(this.options, options);
    }
    StorageBag.prototype.on = function (event, callback) {
        this.listeners[event].push(callback);
    };
    StorageBag.prototype.fireEvent = function (event, params) {
        var _this = this;
        if (params === void 0) { params = []; }
        this.listeners[event].forEach(function (listener) { return listener.apply(_this, params); });
    };
    StorageBag.prototype.get = function (key, defaultReturn, options) {
        ...
        var meta = StorageMeta.fromString(this.provider.getItem(key + ':meta'));
    ...
    StorageBag.prototype.set = function (key, val, options) {
        if (options === void 0) { options = {}; }
        var meta = StorageMeta.create(this.options).merge(options);
        if (meta.isJSON()) {
